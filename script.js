...
const rateUpdatedAt = "2026-03-18 UTC";
...
function convertCurrency() {
  ...
  const amount = Number(state.currentInput);
  const from = fromCurrency.value;
  const to = toCurrency.value;

  if (!Number.isFinite(amount) || !currencyToJpy[from] || !currencyToJpy[to]) {
    return;
  }

  const inJpy = amount * currencyToJpy[from];
  const converted = inJpy / currencyToJpy[to];

  state.currentInput = formatNumber(converted);
  state.storedValue = null;
  state.operator = null;
  state.shouldResetInput = true;
  history.textContent = `${formatNumber(amount)} ${from} → ${to} (固定レート: ${rateUpdatedAt})`;
  screen.textContent = state.currentInput;
}
